...
// @ts-nocheck
	...
	onMount(() => {
		...
		future_days = localStorage.getItem('future_days');

		predict(symbol,start_date,end_date,future_days)
...
<section>
		...
		<div class="input-container">
			<button class="btn btn-filled-primary" 
					...
					on:click={() => predict(symbol, start_date, end_date, future_days)}>
				Predict Stock Price
			</button>
		</div>
	</div>
...
<style>
	...
	.main-content {
		flex: 1; /* The other two columns will share equally the rest of the space */
	}
